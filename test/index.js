...
describe("clusterix", () => {
  ...
  const initializeTestNodes = (length = 3) =>
  ...
  it("should properly create cluster of 3 nodes", async () => {
    ...
    return clusterInstances.map((cluster) =>
      expect(cluster.nodes).to.become(["node1", "node2", "node3"]),
    );
  });
  ...
  it("should throw when initializing two nodes with the same id", async () => {
    ...
    ).to.be.rejectedWith(Error, "Duplicate node sending heartbeats");
  });
  ...
  it("should detect dead node", async () => {
    ...
    return clusterInstances.map((cluster) =>
      expect(cluster.nodes).to.become(["node1", "node2"]),
    );
  });
  ...
  it("should throw when the heartbeat is too big", () => {
    ...
    ).to.throw(Error, "Heartbeats should be more frequent than the timeout");
  });
  ...
  it("should not use an empty id", async () => {
    ...
    return expect(redis.hgetall("heartbeats")).to.become({
      node1: date.toString(),
    });
  });
  ...
  it("should use an id", async () => {
    ...
    return expect(redis.hgetall("mycluster:heartbeats")).to.become({
      node1: date.toString(),
    });
  });
});
